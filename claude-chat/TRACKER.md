...
| 3 | **Progress export/import** | 🔨 **Specced + assigned 2026-08-17** | Parent-zone download/restore of progress as local JSON. Replaces cloud backup for MVP; built on the existing `progressStore` seam. Zero server, zero personal data. **v1 item, not a nice-to-have** — device-local PWA progress is fragile (clear-data / new phone / uninstall). **Shape LOCKED in DECISIONS 2026-08-17:** the progress payload travels and nothing else (the parent passcode is not progress and never enters the file); envelope is versioned and refusable; import **REPLACES rather than merges**, behind a two-step confirm; validate fully, then write once; unknown `skillId`s ignored and counted, never fatal; envelope guarded against an allowlist so it cannot silently widen. Delivery is web-standard — **the Capacitor gap is named and deliberately not built for.** **Owes the policy a sentence in the SAME commit:** the approved draft's *"You can save a backup copy from the Parent Zone at any time"* was deliberately left OUT because the feature does not exist. Restore it in `src/config/privacyPolicy.js`, **FLIP** the `does not promise the unshipped progress export` assertion in `config/__tests__/privacyPolicy.test.js` to assert the opposite — **do not delete it, a deleted guard is a hole** — and regenerate `public/privacy.html` (`npm run privacy:build`) or `privacy:check` goes red on byte identity. **Unblocks** the Netlify rename row in the pre-launch checklist. |
