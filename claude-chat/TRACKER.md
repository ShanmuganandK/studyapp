...
## 🚀 LAUNCH ORDER (set 2026-09-23 — DECISIONS 2026-09-23)

| # | Item | Owner | Note |
|---|---|---|---|
| L1 | Move to `tinku.stringus.com` | Human (DNS + Netlify), Code (doc placeholders) | First, before anyone new gets a link. Testers export → new URL → import. |
| L2 | Play Console account + identity verification | Human | Start now; verification takes time. Developer name = policy operator name. |
| L3 | Grade picker + Grades 1–2 description | Code | ✅ **Done 2026-09-26** — Task B, `5f0050e`+`5ae9cb6` on `master`. |
| L4 | Android build — status unknown, investigate | Code (report), then human decision | Does a buildable Android project exist? |
| L5 | Phone regression walk on the new domain | Human | `phoneregressionchecklist.pdf`, section 0 first. |
| L6 | Operator line filled → `npm run privacy:build` | Human decides, Code applies | Must match the Play developer name. |
| L7 | Store listing, Data Safety entry, content rating, target age (6–8), Designed-for-Families | Human | Answers drafted in `play-data-safety-form.md`. |
| L8 | Success bar written down | Human | Before the public link. E.g. testers still active on day 14. |
| L9 | Closed test: 12 testers × 14 days = the teaser | Human | Then apply for production. |

Supersedes the 2026-08-21 trip priority order (git history holds it).
...
| 5 | Screen 3-B verdict (journey path vs. cards) | ⏳ **Post-launch** | Judge on current (post-grammar-fix) build. Path is live on master; card view at `?home=cards`. Kid-testing is the gate — it is happening now. |
...
| Play Console account | ⏳ **Not created — L2** | DECISIONS says individual account under UAE identity, payouts to UAE bank. **Is it actually created and verified?** Google identity verification for individual developers takes time — start it early even though MVP takes no money. **Personal account, not yet created — L2.** |
...
| Item | Status | Detail |
|---|---|---|
| **CA / tax questionnaire — SEND** | ⏳ Still live | `claude-chat/questionnaire-ca-tax-uae-india.md`. **Not** deferred with the DPDP consult. Section D1 (India day-count before UAE income is at risk of Indian residence taxation) is time-sensitive. **Status: human to confirm.** |
| **India day-count log** | ⏳ **Status: human to confirm** | Track days-in-India from arrival, independent of when the CA replies. |
| Kid-testing in India | ⏸ **Not a launch gate (DECISIONS 2026-09-23); resumes post-launch** | Signal source for the 3-B verdict (#5), the welcome-screen decision (#9), FRONTIER_PICK/review-embedding (#6 gaps), theme reaction (#10), and the revisit trigger above. See the Kid-Test Log watch-list. |
| Teacher review of `misconceptions-reference.md` | ⏳ Pending — **arrange while in India** | ~68 rows, one-time. Unblocks richer dashboard insight AND is a prerequisite for the non-arithmetic recipes (#7 remainder), whose distractor rules for shapes/time/money almost certainly are not in the doc yet. **No longer a launch gate (DECISIONS 2026-09-23).** |
...
## Done — First-run grade picker + Grades 1–2 description (2026-09-26)

Launch order **L3** (Task B). Implements `DECISIONS.md` 2026-09-23 (launch scope — LOCKED) and
closes **Now #9**, the welcome/onboarding question that had been held open for kid-test signal
since 2026-08-18. Branch `launch-grade-picker`, merged fast-forward into `master`
(`de36b9f..5ae9cb6`).

**What shipped.**
- **`GradePickerScreen.jsx`** — shown once, before anything else, on a device that has never had
  test settings written. Asks one parent-facing question ("Which class is your child in?" —
  Class 1 / Class 2 / Class 3 disabled "coming soon") and nothing else: no name, no age, no
  accounts. Presentational; the hook does the writing.
- **`gradeChosen`** in `tinku:v1:testSettings` — the flag that distinguishes "never asked" from
  "a parent deliberately chose Class 1", which the stored grade alone cannot do (it defaults to 1).
  `SCHEMA_VERSION` deliberately NOT bumped, same reasoning as `bridgeEnabled`.
- **`chooseInitialGrade(n)`** — the picker's only write path, setting grade + flag atomically. The
  parent-zone `setGrade` deliberately does NOT touch the flag, so changing grade later can never
  re-trigger the picker.
- **`ThemeManager`** returns the picker INSTEAD of the whole `Layout` tree, so there is no nav
  chrome behind it and no separate "continue" step — the next render is the app proper on the
  chosen grade.
- **Grade 3 shown, disabled,** in the test panel too ("Grade 3 — soon"), so the row does not just
  stop at 2 with no trace of a third option. `GRADES` itself stays `[1, 2]`.
- **`DESCRIPTION` now reads "Grades 1-2"**, guarded both directions (1-2 present, 1-3 absent), so
  the wider claim cannot return silently ahead of the content. This closes the ⚠️ warning the
  app-name checklist row has carried since 2026-08-21.

**The backfill direction is the load-bearing detail.** It runs OPPOSITE to `bridgeEnabled`'s: a
stored record MISSING the key is an existing tester who already has a grade, so it backfills
**true** — they are never re-asked. Only a device with no record at all yields false.

**Does NOT** revive `ProfileSetup.jsx` / `ProfileSelector.jsx` — both stay quarantined and
unrendered. The grade still never enters a progress export (verified across modules), and the
`tinku:v1:testSettings` key is NOT renamed — renaming resets every device.

**Guards proven RED before being trusted** (standing rule): inverting the backfill fails the
returning-tester test; forcing the gate off fails the three "sees the picker" tests; forcing it on
fails the four "never sees the picker" tests. The gate test's assertions are async on purpose —
`AuthProvider` renders `{!loading && children}`, so a synchronous absence-check passes on an empty
DOM, and the first draft of that file "passed" two negatives vacuously before this was caught.

**Verification:** full CI chain green locally — `lint` 0 errors (3 pre-existing warnings),
`lint:hex` clean, `privacy:check` up to date, **534 passed + 1 skipped** (524 before), `build` OK,
post-build bundle guard green. `ARCHITECTURE.md` updated in the same commit as the code.

**Not done here (human):** L1's DNS/Netlify move, L2's Play account, and the phone walk (L5) —
the picker has not yet been seen on a real device.
