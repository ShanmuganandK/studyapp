 ...
 * and the toggle defaulting false. Also covers `gradeChosen` (DECISIONS 2026-09-23) — the
 * OPPOSITE backfill direction from `bridgeEnabled`: a legacy record with no `gradeChosen` key
 * backfills to TRUE (already chosen), while a genuinely empty/corrupt/version-mismatched read
 * backfills to FALSE (never chosen — the only path that shows the first-run picker).
 */
...
describe('testSettings', () => {
  ...
  it('returns defaults when nothing is stored', async () => {
    ...
    expect(DEFAULT_TEST_SETTINGS).toEqual({ theme: 'wonder', grade: 1, bridgeEnabled: false, gradeChosen: false });
  });
  ...
  it('round-trips a saved theme + grade + bridgeEnabled', async () => {
    ...
    expect(loadTestSettings()).toEqual({ theme: 'deepsea', grade: 2, bridgeEnabled: true, gradeChosen: true });
  });
  ...
  it('normalises grade 3 back to the default — no Grade-3 curriculum yet (Now #12)', async () => {
    ...
    expect(loadTestSettings()).toEqual({ theme: 'sunset', grade: 1, bridgeEnabled: false, gradeChosen: true });
  });
  ...
  it('normalises a non-boolean bridgeEnabled back to the default (false)', async () => {
    ...
    expect(loadTestSettings()).toEqual({ theme: 'sunset', grade: 2, bridgeEnabled: false, gradeChosen: true });
  });

  it('normalises a non-boolean gradeChosen back to the default for a fresh save (true, existingRecord)', async () => {
    const { loadTestSettings, saveTestSettings } = await import('../testSettings.js');
    saveTestSettings({ theme: 'sunset', grade: 2, gradeChosen: 'yes' });
    expect(loadTestSettings()).toEqual({ theme: 'sunset', grade: 2, bridgeEnabled: false, gradeChosen: true });
  });
  ...
  it('returns defaults when the stored version does not match', async () => {
    ...
    expect(loadTestSettings()).toEqual({ theme: 'wonder', grade: 1, bridgeEnabled: false, gradeChosen: false });
  });
  ...
  describe('legacy load (DECISIONS 2026-09-22 — SCHEMA_VERSION deliberately NOT bumped)', () => {
    it('a pre-existing v1 object with NO bridgeEnabled key keeps its theme and grade, toggle defaults false', async () => {
      ...
      expect(loadTestSettings()).toEqual({ theme: 'bubblegum', grade: 2, bridgeEnabled: false, gradeChosen: true });
    });

    it('a pre-existing v1 object with NO gradeChosen key is treated as already chosen (opposite backfill from bridgeEnabled)', async () => {
      const { loadTestSettings } = await import('../testSettings.js');
      storage.setItem(KEY, JSON.stringify({ version: 1, theme: 'sunset', grade: 1, bridgeEnabled: true }));
      expect(loadTestSettings().gradeChosen).toBe(true);
    });

    it('a genuinely fresh device (no record at all) is the only path where gradeChosen is false', async () => {
      const { loadTestSettings } = await import('../testSettings.js');
      expect(loadTestSettings().gradeChosen).toBe(false);
    });
    ...
    it('an export-shaped read of a device with the toggle ON contains no trace of it (device-only, never travels)', async () => {
      ...
      const { saveTestSettings } = await import('../testSettings.js');
      saveTestSettings({ theme: 'wonder', grade: 1, bridgeEnabled: true, gradeChosen: true });
