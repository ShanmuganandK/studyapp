...
- (2026-08-25) **Distractor plausibility: at most one implausible option per question (LOCKED).**
  ...
  **no entry in `misconceptions-reference.md`**. It was correctly not invented in code. **This is
  now the top item for the teacher review.** *(Closed 2026-09-21/22 — see the `ones-addition-ignored`
  entries in `TRACKER.md`; teacher sign-off on the rule itself is `TEACHER-REVIEW.md` item 4, still
  open.)*
...
- (2026-09-22) **Remediation ladder, step 3: session-level park + one bonus question, excluded from scoring (LOCKED).**

  **The gap.** The Learning-engine bullet's spec — wrong #1 = hint, wrong #2 = visual walkthrough +
  easier retry, wrong #3 = park + guaranteed-win — was never fully built. What shipped
  (`useQuizSession.js`) is wrong #1 = hint (same question stays live), wrong #2 = reveal the correct
  answer with a learning-tone fill, auto-advance. No walkthrough, no easier retry, no park, no bonus
  question. First flagged 2026-08-26, untouched since.

  **Wrong #2 is not being changed, and the reason is structural, not scope-cutting.** A session runs
  at one fixed difficulty for its whole 8-question run — there is no easier version of the same
  skill to retry into mid-session without changing session composition itself, which is a separate,
  deliberately-unsettled question (the "does a session walk rungs" thread left open in the
  2026-08-27 entry). So "wrong #2 = easier retry" is corrected here to match what the shape of the
  session actually allows, rather than built to match spec text that assumed a session-composition
  change nobody decided to make.

  **What step 3 is, concretely.** Any wrong #2 (a reveal) during a session flags that session
  **parked** — a single boolean, set once, regardless of how many reveals occur. Parking is resolved
  **only at session end**, never mid-session: the running 8 scored questions are never interrupted or
  re-ordered by a reveal. If the session ends parked, **exactly one bonus question** is appended
  after the 8th scored question and before the session completes, drawn at the skill's **easiest**
  difficulty. The bonus question runs the ordinary ladder mechanics (a wrong tap still gets a hint)
  but its outcome is **excluded entirely** from `questionsTotal`, `questionsCorrect`, score, and the
  `sessionResult` shape mastery's `applyResult` consumes — mastery must not be able to tell a parked
  session happened.

  **Why one bonus question, not one per reveal, and why excluded from scoring.** This is the mood
  floor named in the Learning-engine bullet ("a session never ends on failure") and an application of
  the average-learner principle (2026-08-25): the child this product is for is exactly the one who
  needed the reveal, and ending their session there — even under the app's always-celebratory closing
  screen — leaves their last live action a failure. One guaranteed-easy win, deliberately kept out of
  the score, restores that without inflating the mastery signal the way a scored bonus question would
  (inflated scores drive premature promotion — the same harm the 2026-08-25 plausibility fix and the
  2026-09-01/09-02 level-streak episode both turned on).

  **Scope.** `g2.add.2d-nocarry` first or universal — Code's call at implementation, since the
  mechanism (session-level flag + a post-scoring bonus phase in `useQuizSession`) is skill-agnostic
  by construction; record which in `TRACKER.md`.
