...
- (2026-07-04) **Answer feedback fills the blank (LOCKED):** for any format that renders a blank (`compare` now; `text-input` + future blank-bearing formats later), a CORRECT answer lands the committed correct value into the blank as part of the correct-beat, so the child sees the full statement complete before transition. Formats without a blank (`mcq`, `count-objects`) keep option-pop feedback — no fill. Presentation-only, view-state only; never plumb new data from the hook/recipe for a visual beat. Resting/wrong/hint states show the placeholder (never slot a wrong value in).
